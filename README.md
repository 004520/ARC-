...
基于 ArcFace 深度学习算法的人脸识别系统，集成 GUI 可视化界面、实时摄像头识别、人脸库管理和考勤打卡统计功能，使用 PyTorch + Tkinter + MySQL 构建。
...
- 支持多人同时识别，每张人脸标注姓名和置信度
- 识别结果实时叠加在摄像头画面上
- 未检测到人脸时显示半透明提示文字
- 陌生人警告：未识别的人脸以红色框标注并显示"陌生人"提示，已知人脸以绿色框标注并显示姓名和相似度
...
计算机看人脸，本质是把一张照片转换成一个特征向量（一串数字）。如果同一个人的两张照片，它们的特征向量足够"像"（余弦相似度高），计算机就认为这是同一个人。

普通的分类网络（比如 Softmax）只能学会区分不同的人，但它不保证同一类人的特征向量很紧凑。打个比方：Softmax 只是把每个人划到一个小格子，但格子里的人可能站得很散；ArcFace 则是把同一个格子里的人往中心推，同时把不同格子推得更远。
...
ArcFace 的核心创新：在正确类别的角度 theta_y 上额外加一个间隔 m，让模型必须把特征向量推得更靠近类中心才能正确分类。用大白话说就是——增加了"及格线"的难度：

$$L_{\text{arc}} = -\frac{1}{N}\sum_{i=1}^{N}\log\frac{e^{s \cdot \cos(\theta_{y_i} + m)}}{e^{s \cdot \cos(\theta_{y_i} + m)} + \sum_{j \neq y_i} e^{s \cdot \cos\theta_j}}$$

关键实现细节

编程时不能用 cos(theta + m) 直接算，需要用三角恒等式展开：

$$\cos(\theta + m) = \cos\theta \cdot \cos m - \sin\theta \cdot \sin m$$

其中 sin(theta) = sqrt(1 - cos^2(theta))（因为归一化后向量在单位球面上）。
...
为什么不用论文的 s=64？论文基于百万级数据集（MS-Celeb-1M），本项目使用千级数据集（CASIA 500人），过大的 s 会导致初始损失爆炸、梯度稀疏。将 s 降到 30 后，初始损失从约 37 降至约 21，训练稳定。

3.3 模型架构

整体结构

模型基于 IR-ResNet（Improved Residual Network），这是 ArcFace 论文推荐的骨干网络。一张 112x112 的人脸照片经过以下流程变成 512 维特征向量：

```
输入 (3x112x112)
  |
  v  Conv2d 3->64, BN + PReLU
  |
  v  Layer1: 3 个 IRBlock, 64 通道 -> 28x28
  |
  v  Layer2: 4 个 IRBlock, 128 通道 -> 14x14
  |
  v  Layer3: 14 个 IRBlock, 256 通道 -> 7x7
  |
  v  Layer4: 3 个 IRBlock, 512 通道 -> 4x4
  |
  v  BN -> 1x1 Conv -> BN -> PReLU -> GAP -> Dropout -> L2 Norm
  |
  输出: 512 维归一化特征向量
```
...
> Flatten(4x4x512) -> FC(8192, 512) = 419 万参数 (不可取)

本项目改用 1x1 卷积 + 全局平均池化（GAP）：

> 1x1 Conv(512, 512) -> GAP -> 26 万参数 (减少 94%)
